...
one by one; the other two are Phase 35's. The other 1,043 sat in the
benchmark's sixteen-repository reserve, 1,019 of them in one repository,
which no pass had read because the reserve is held for exactly that
evaluation and reading it spends it. The operator spent that one
repository's reserve on the reading rather than publish an unjudged
number: all 1,019 are raw-HTML links in a DevSite tree the site detection
deliberately does not reach, 1,003 of them routes that resolve once the
extension is appended, 15 in a generated index written a directory too
shallow, one a real broken image in a vendored README - the known
undetected-generator class and no new shape. The manifest records the
opening, and the 24 additions in four other reserve repositories stay
unread. The report's provenance section carries all of it, computed from
the reading records rather than asserted. The recording instrument now
decodes a SARIF URI back to a path, because the 0.26.1 writer
percent-encodes one as the spec asks and every join downstream is on the
tracked path.

**What this leaves owed.** The heldout and niche reserves were not opened,
and 24 reserve additions on the benchmark are unread. `AGENTS.md` and
`CONTRIBUTING.md` still state
