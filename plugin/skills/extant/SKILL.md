...
| `dead-md-link` | `[text](path)`, `[text](path "title")`, `[text](<a path>)`, `[label]: path` or an HTML `href`/`src` whose file is gone | whole file; a Rust path in markdown rustdoc includes is left to rustdoc |
...
| `dead-line-pointer` | `file.py:123` or `file.py:100-250` where that file has fewer lines | files this repository tracks |
