...
| `dead-md-link` | `[a link](to/a/file.md)` whose target is gone; also a `[label]: file.md` definition and the `href` or `src` of a raw HTML tag. A link with a title, `[a link](file.md "Title")`, and one spelled `[a link](<a file.md>)` are read the way CommonMark reads them. A Rust path such as `[x]: crate::Item` in markdown that `#[doc = include_str!]` pulls into rustdoc is left to rustdoc |
...
```yaml
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      - uses: scooter-sensei/extant@v0.26.1
```
