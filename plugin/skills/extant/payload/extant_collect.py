...
# The dead-line-pointer rule is extant/rules/line_pointer.py now, and
# `_POINTER_SITES` went with it: the memo is shared by the rule and its own
# denominator, which is exactly why extant/scope.py could not make it a RunScope
# field. `validate()` and `run_scope()` below still drop it, through the
# module's own `_forget_sites()`.
from extant.rules import line_pointer as _rule_line

from extant.rules.line_pointer import (   # noqa: F401  (re-exported as they
    _LINE_COUNT_LIMIT, _LINE_POINTER,      # are: a pattern and a bound)
)


def _line_count(repo: Path, relative: str) -> int | None:
    """Lines in a tracked file, or None when it cannot be counted here."""
    return _rule_line._line_count(_ctx(repo), relative)
...
# Rendering findings - text, GitHub annotations, SARIF - and the baseline file
# is extant/report.py now. Nothing there reads ambient state, so every name is
# re-exported as it is; `_fingerprint` lost its underscore over there because
# `main()` below is a different module now and calls it across the boundary.
from extant.report import (          # noqa: F401  (re-exported as they are:
    BASELINE_NAME, FORMATS,          # pure formatters, no state)
    format_github, format_sarif, format_text, load_baseline, render_findings,
    write_baseline,
)
from extant.report import (          # noqa: F401  (promoted for a sibling
    fingerprint as _fingerprint,     # that calls it, re-exported here under
)                                    # the old spelling)
from extant.report import _utf16_len   # noqa: F401  (the suite reads it)
